...
// 定义要输出的文件
#define OUTPUT_FILE "/home/wangc/lab3/part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffffb14b0780;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffffb1494160;
...
static unsigned long virt2phys(struct mm_struct *mm, unsigned long virt)
{
    ...
    return pte_pfn(*pte); // 根据pte找出物理地址
}
...
static void print_seg_info(void)
{
    ...
    //struct page *start_code_page = mfollow_page(vm_area, virt_addr, FOLL_GET);
