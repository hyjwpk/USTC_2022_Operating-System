...
int write_fat_entry(FAT16 *fat16_ins, WORD clusterN, WORD data) {
  ...
  for (uint i = 0; i < fat16_ins->Bpb.BPB_NumFATS; i++) {
      ...
      sector_read(fat16_ins->fd, FirstFatSecNum + fat16_ins->Bpb.BPB_FATSz16 * i + ClusterSec, SectorBuffer);
      memcpy(SectorBuffer + SecOffset, &data, 2);
      sector_write(fat16_ins->fd, FirstFatSecNum + fat16_ins->Bpb.BPB_FATSz16 * i + ClusterSec, SectorBuffer);
      /*** END ***/
  }
  return 0;
}
...
WORD alloc_clusters(FAT16 *fat16_ins, uint32_t n)
{
  ...
  /*** BEGIN ***/
  WORD ClusterN = CLUSTER_MIN;
  while (allocated < n && ClusterN <= CLUSTER_MAX) {
      if (fat_entry_by_cluster(fat16_ins, ClusterN) == CLUSTER_FREE) {
  ...
  for (uint32_t i = 0; i < n; i++) {
      ...
      for (int DirSecCnt = 0; DirSecCnt < fat16_ins->Bpb.BPB_SecPerClus; DirSecCnt++) {
          memset(sector_buffer, 0, BYTES_PER_SECTOR);
          sector_write(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, sector_buffer);
      }
      write_fat_entry(fat16_ins, ClusterN, clusters[i + 1]);
  }
  /*** END ***/
...
int fat16_mkdir(const char *path, mode_t mode) {
  ...
  fprintf(logfile,"[fat16_mkdir begin]\n");
  fprintf(logfile,"path: %s, mode: %u\n",path,mode);
  ...
  if (strcmp(prtPath, "/") == 0) {
      ...
      // 依次读取根目录中每个目录项，目录项的最大条数是RootEntCnt
      for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++) {
          // 这段代码中，从扇区数据（存放在sector_buffer）中正确位置位置读取到相应的目录项，
          memcpy(&Root, sector_buffer + ((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR, BYTES_PER_DIR);
          //若目录为空或目录为已删除则找到项
          if (findFlag == 0 && (Root.DIR_Name[0] == 0x00 || Root.DIR_Name[0] == 0xE5)) {
  ...
  /* Else if parent directory is sub-directory */
      ...
      if (find_root(fat16_ins, &Dir, prtPath, &offset_dir)) {
          fprintf(logfile,"[fat16_mkdir end]\n\n");
      ...
      for (uint i = 1;; i++) {
          ...
          //若目录为空或目录为已删除则找到项
          if (findFlag == 0 && (Dir.DIR_Name[0] == 0x00 || Dir.DIR_Name[0] == 0xE5)) {
  ...
  if (findFlag == 1) {
    ...
    dir_entry_create(fat16_ins,sectorNum,offset,paths[pathDepth-1],ATTR_DIRECTORY,ClusterN,0);
    first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);
    dir_entry_create(fat16_ins,FirstSectorofCluster,0,".",ATTR_DIRECTORY,CLUSTER_END,0);
    dir_entry_create(fat16_ins,FirstSectorofCluster,BYTES_PER_DIR,"..",ATTR_DIRECTORY,CLUSTER_END,0);
    /*** END ***/
  }
  fprintf(logfile,"[fat16_mkdir end]\n\n");
  pthread_mutex_unlock(&mtx);
...
void dir_entry_delete(FAT16 *fat16_ins, off_t offset) {
  ...
  uint ClusterSec;
  uint SecOffset; 
  ClusterSec = offset / fat16_ins->Bpb.BPB_BytsPerSec;
  SecOffset = offset % fat16_ins->Bpb.BPB_BytsPerSec;
  BYTE zero = 0xe5; 
  sector_read(fat16_ins->fd, ClusterSec, buffer);
  memcpy(buffer + SecOffset, &zero, 1);
  sector_write(fat16_ins->fd,ClusterSec, buffer);
  /*** END ***/
}
...
void dir_entry_write(FAT16 *fat16_ins, off_t offset, const DIR_ENTRY *Dir) {
  ...
  SecOffset = offset % fat16_ins->Bpb.BPB_BytsPerSec;
  sector_read(fat16_ins->fd, ClusterSec, buffer);
  memcpy(buffer + SecOffset, Dir, BYTES_PER_DIR);
  sector_write(fat16_ins->fd,ClusterSec, buffer);
  /*** END ***/
}
...
int fat16_rmdir(const char *path) {
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_rmdir begin]\n");
  fprintf(logfile,"path: %s\n",path);
  /* Gets volume data supplied in the context during the fat16_init function */
  FAT16 *fat16_ins = get_fat16_ins_fix();

  if(strcmp(path, "/") == 0) {
    fprintf(logfile,"[fat16_rmdir end]\n\n");
    pthread_mutex_unlock(&mtx);
  ...
  /* Start searching the root's sub-directories starting from Dir */
  for (uint i = 1;; i++) {
      memcpy(&curDir, sector_buffer + ((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR, BYTES_PER_DIR);
      //若目录不为空或已删除
      if (curDir.DIR_Name[0] != 0x00 && curDir.DIR_Name[0] != 0xE5) {
          //若属性为目录、文件
          if ((curDir.DIR_Attr == ATTR_DIRECTORY || curDir.DIR_Attr == ATTR_ARCHIVE)) {
              //若不是“.”或者“..”
              if (strncmp((char *)curDir.DIR_Name, ".", 1) && strncmp((char *)curDir.DIR_Name, "..", 2)) {
                  fprintf(logfile,"[fat16_rmdir end]\n\n");
                  pthread_mutex_unlock(&mtx);
                  return -ENOTEMPTY;
              }
  ...
  while (ClusterN != 0xffff && ClusterN >= 0x0002) {
...
size_t write_to_cluster_at_offset(FAT16 *fat16_ins, WORD clusterN, off_t offset, const BYTE* data, size_t size) {
   ...
   */
  /*** BEGIN ***/
  size_t DirSecCnt = offset / BYTES_PER_SECTOR;
  size_t sector_offset = offset % BYTES_PER_SECTOR;
  ...
  while (write_offset < size) {
      // 将下一个扇区的数据读入sector_buffer
      sector_read(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, sector_buffer);
      size_t copy = (BYTES_PER_SECTOR - sector_offset <= size - write_offset) ? (BYTES_PER_SECTOR - sector_offset) : (size - write_offset);
      memcpy(sector_buffer + sector_offset, data + write_offset, copy);
      sector_write(fat16_ins->fd, FirstSectorofCluster + DirSecCnt, sector_buffer);
      write_offset += copy;
      sector_offset = 0;
      DirSecCnt++;
...
WORD file_last_cluster(FAT16 *fat16_ins, DIR_ENTRY *Dir, int64_t *count) {
  ...
  /*** BEGIN ***/
  WORD ClusterN = Dir->DIR_FstClusLO; //目录项中存储了我们要读取的第一个簇的簇号
  //找到起始簇
  while (is_cluster_inuse(ClusterN)) {
      cnt++;
      cur = ClusterN;
...
int file_new_cluster(FAT16 *fat16_ins, DIR_ENTRY *Dir, WORD last_cluster, DWORD count)
{
   ...
   */
  /*** BEGIN ***/
  WORD clustersN = alloc_clusters(fat16_ins, count);
  if (last_cluster == CLUSTER_END) {
      Dir->DIR_FstClusLO = clustersN;
  } else {
      write_fat_entry(fat16_ins, last_cluster, clustersN);
  }
  /*** END ***/
  return last_cluster;
}
...
int write_file(FAT16 *fat16_ins, DIR_ENTRY *Dir, off_t offset_dir, const void *buff, off_t offset, size_t length)
{
  ...
  if (Dir->DIR_FileSize > cur_cluster_count * fat16_ins->Bpb.BPB_SecPerClus * BYTES_PER_SECTOR) {
      DWORD count = (Dir->DIR_FileSize + fat16_ins->ClusterSize - 1) / fat16_ins->ClusterSize - cur_cluster_count;
      file_new_cluster(fat16_ins, Dir, last_cluster, count);
  }
  /*** END ***/
  ...
  size_t begin_cluster = beginBytes / (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus);

  WORD ClusterN;                 // 当前读取的簇号
  ClusterN = Dir->DIR_FstClusLO; //目录项中存储了我们要读取的第一个簇的簇号

  //找到起始簇
  for (size_t i = 0; i < begin_cluster; i++) {
      if (ClusterN == 0xffff) {
          return 0;
      }
      ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
  ...
  size_t write_offset = 0;
  size_t cluster_offset = offset % (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus);

  while (write_offset < length) {
      size_t copy = (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus - cluster_offset <= length - write_offset) ? (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus - cluster_offset) : (length - write_offset);
      write_to_cluster_at_offset(fat16_ins, ClusterN, cluster_offset, buff + write_offset, copy);
      ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
      cluster_offset = 0;
      write_offset += copy;
  }
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_write begin]\n");
  fprintf(logfile,"path: %s, data: ",path);
  fwrite(data,(size < 30 ? size : 30),1,logfile);
  fprintf(logfile,", size: %ld, offset: %ld\n",size,offset);
  FAT16 *fat16_ins = get_fat16_ins_fix();
  ...
  size_t length = write_file(fat16_ins, &Dir, offset_dir, data, offset, size);
  /*** END ***/
  fprintf(logfile,"[fat16_write end]\n\n");
  pthread_mutex_unlock(&mtx);
  return length;
}
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_truncate begin]\n");
  fprintf(logfile,"path: %s, size: %ld\n",path,size);
  /* Gets volume data supplied in the context during the fat16_init function */
  FAT16 *fat16_ins = get_fat16_ins_fix();
  ...
  if (old_size == new_size){
    fprintf(logfile,"[fat16_truncate end]\n\n");
    pthread_mutex_unlock(&mtx);
    return 0;
    ...
    off_t offset = old_size % fat16_ins->ClusterSize;
    size_t size = fat16_ins->ClusterSize - offset;
    BYTE *data = (BYTE *)malloc(size);
    memset(data,0,size);
    write_to_cluster_at_offset(fat16_ins,last_cluster,offset,data,size);
    dir_entry_write(fat16_ins,offset_dir,&Dir);
    /*** END ***/
  } else {  // 截断文件
      ...
      for (size_t i = 0; i < new_cluster_count; i++) {
          cur_ClusterN = ClusterN;
          ClusterN = fat_entry_by_cluster(fat16_ins, ClusterN);
      }
      if (new_cluster_count == 0) {
          Dir.DIR_FstClusLO = CLUSTER_END;
      } else {
          write_fat_entry(fat16_ins, cur_ClusterN, CLUSTER_END);
      }
      while (is_cluster_inuse(ClusterN)) {
