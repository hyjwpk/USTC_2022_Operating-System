...
void *fat16_init(struct fuse_conn_info *conn)
{
  ...
  fprintf(logfile,"[fat16_init begin]\n");
  fprintf(logfile,"conn: %p\n",conn);
  struct fuse_context *context;
  context = fuse_get_context();
  fprintf(logfile,"[fat16_init end]\n\n");
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_destroy begin]\n");
  fprintf(logfile,"data: %p\n",data);
  free(data);
  fprintf(logfile,"[fat16_destroy end]\n\n");
  pthread_mutex_unlock(&mtx);
}
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_getattr begin]\n");
  fprintf(logfile,"path: %s, stbuf: %p\n",path,stbuf);
  ...
  else
  {
    ...
    if (res == 0)
    {
      ...
      fprintf(logfile,"[fat16_getattr end]\n\n");
      pthread_mutex_unlock(&mtx);
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_readdir begin]\n");
  fprintf(logfile,"path: %s, buffer: %p, filler: %p, offset: %ld\n",path,buffer,filler,offset);
  FAT16 *fat16_ins = get_fat16_ins();
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      if (i % 16 == 0)
      {
        ...
        else // 当前簇已经读完，需要读取下一个簇的内容
        {
          ...
          if (FatClusEntryVal == 0xffff)
          {
            fprintf(logfile,"[fat16_readdir end]\n\n");
            pthread_mutex_unlock(&mtx);
            return 0;
          }
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_read begin]\n");
  fprintf(logfile,"path: %s, buffer: %p, size: %ld, offset: %ld\n",path,buffer,size,offset);
  ...
  if (find_root(fat16_ins, &Dir, path, &offset_dir)) {
      fprintf(logfile,"[fat16_read end]\n\n");
      pthread_mutex_unlock(&mtx);
      return 0;
  }
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_mknod begin]\n");
  fprintf(logfile,"path: %s, mode: %d, devNum: %ld\n",path,mode,devNum);
  /* Gets volume data supplied in the context during the fat16_init function */
  FAT16 *fat16_ins = get_fat16_ins();
  ...
  fprintf(logfile,"[fat16_mknod end]\n\n");
  pthread_mutex_unlock(&mtx);
  return 0;
}
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_unlink begin]\n");
  ...
  {
    fprintf(logfile,"[fat16_unlink end]\n\n");
    pthread_mutex_unlock(&mtx);
    return 1;
  }
...
{
  pthread_mutex_lock(&mtx);
  fprintf(logfile,"[fat16_utimens begin]\n");
  fprintf(logfile,"path: %s\n",path);
  fprintf(logfile,"[fat16_utimens end]\n\n");
  pthread_mutex_unlock(&mtx);
  return 0;
}
