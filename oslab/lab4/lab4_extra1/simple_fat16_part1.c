...
{
  BYTE sector_buffer[3][BYTES_PER_SECTOR];
  fseek(fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
  fread(sector_buffer[0], BYTES_PER_SECTOR, 1, fd);
  fseek(fat16_ins2_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
  fread(sector_buffer[1], BYTES_PER_SECTOR, 1, fat16_ins2_fd);
  fseek(fat16_ins3_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
  fread(sector_buffer[2], BYTES_PER_SECTOR, 1, fat16_ins3_fd);
  int equal[3];
  equal[0] = memcmp(sector_buffer[0],sector_buffer[1],BYTES_PER_SECTOR);
  equal[1] = memcmp(sector_buffer[0],sector_buffer[2],BYTES_PER_SECTOR);
  equal[2] = memcmp(sector_buffer[1],sector_buffer[2],BYTES_PER_SECTOR);
  if(equal[0] != 0 && equal[1] != 0 && equal[2] != 0){
    exit(-1);
  }
  else if(equal[0] == 0 && equal[1] != 0 && equal[2] != 0){
    fseek(fat16_ins3_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fwrite(sector_buffer[0], BYTES_PER_SECTOR, 1, fat16_ins3_fd);
  }
  else if(equal[0] != 0 && equal[1] == 0 && equal[2] != 0){
    fseek(fat16_ins2_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fwrite(sector_buffer[0], BYTES_PER_SECTOR, 1, fat16_ins2_fd);
  }
  else if(equal[0] != 0 && equal[1] != 0 && equal[2] == 0){
    fseek(fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fwrite(sector_buffer[1], BYTES_PER_SECTOR, 1, fd);
  }
  fseek(fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
...
void sector_write(FILE *fd, unsigned int secnum, const void *buffer)
{
  ...
  fseek(fat16_ins2_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
  fwrite(buffer, BYTES_PER_SECTOR, 1, fat16_ins2_fd);
  fseek(fat16_ins3_fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
  fwrite(buffer, BYTES_PER_SECTOR, 1, fat16_ins3_fd);
  fflush(fd);
}
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      if (Root.DIR_Name[0] != 0x00 && Root.DIR_Name[0] != 0xE5) {
          //若属性为目录、文件
          if ((Root.DIR_Attr == ATTR_DIRECTORY || Root.DIR_Attr == ATTR_ARCHIVE)) {
              filler(buffer, (const char *)path_decode(Root.DIR_Name), NULL, 0);
          }
      }
      /*** END ***/
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      if (Dir.DIR_Name[0] != 0x00 && Dir.DIR_Name[0] != 0xE5) {
          ...
          if ((Dir.DIR_Attr == ATTR_DIRECTORY || Dir.DIR_Attr == ATTR_ARCHIVE)) {
              filler(buffer, (const char *)path_decode(Dir.DIR_Name), NULL, 0);
          }
      }
      /*** END ***/
               ...
               struct fuse_file_info *fi)
{
  ...
  size_t begin_sector = (beginBytes / BYTES_PER_SECTOR) % fat16_ins->Bpb.BPB_SecPerClus;
  size_t begin_cluster = beginBytes / (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus);
  size = (size + offset <= Dir.DIR_FileSize) ? size : Dir.DIR_FileSize - offset;
  ...
  while (buffer_offset < size) {
      ...
      size_t copy = (BYTES_PER_SECTOR - sector_offset <= size - buffer_offset) ? (BYTES_PER_SECTOR - sector_offset) : (size - buffer_offset);
      memcpy(buffer + buffer_offset, sector_buffer + sector_offset, copy);
      sector_offset = 0;
      buffer_offset += copy;
  }
  /*** END ***/
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  memcpy(entry_info + 11, &attr, 1 * sizeof(BYTE));
  /*** END ***/
  ...
  memcpy(sector_buffer + offset, entry_info, BYTES_PER_DIR);
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
  ...
  for (uint i = 0; i < fat16_ins->Bpb.BPB_NumFATS; i++) {
      sector_read(fd, FirstFatSecNum + fat16_ins->Bpb.BPB_FATSz16 * i + ClusterSec, sector_buffer);
      memcpy(sector_buffer + SecOffset, buffer, 2);
      sector_write(fd, FirstFatSecNum + fat16_ins->Bpb.BPB_FATSz16 * i + ClusterSec, sector_buffer);
  }
  /*** END ***/
...
int fat16_unlink(const char *path)
{
  ...
  if (findFlag == 1)
  {
    ...
    BYTE attr = 0xE5;
    sector_read(fat16_ins->fd, sectorNum, sector_buffer);
    memcpy(sector_buffer + offset, &attr, 1);
    sector_write(fat16_ins->fd, sectorNum, sector_buffer);
    /*** END ***/
  }
  return 0;
...
int main(int argc, char *argv[])
{
  ...
  fat16_ins2_fd = fopen("fat16_2.img", "rb+");
  fat16_ins3_fd = fopen("fat16_3.img", "rb+");
